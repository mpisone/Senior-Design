...
void MPU6050::getAccRaw(float *x, float *y, float *z) {
...
void MPU6050::getAcc(float *x, float *y, float *z) {
	getAccRaw(x, y, z); //Store raw values into variables
	...
	*ax_off = 0, *ay_off = 0, *az_off = 0; //Initialize the offsets to zero
	...
	for (int i = 0; i < 10000; i++) { //Use loop to average offsets
		...
		*gr_off = *gr_off + gyro_off[0], *gp_off = *gp_off + gyro_off[1], *gy_off = *gy_off + gyro_off[2]; //Add to sum
		...
		getAccRaw(&accel_off[0], &accel_off[1], &accel_off[2]); //Raw accelerometer values
